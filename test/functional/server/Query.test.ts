...
export class QueryTest extends JwtMongooseTestBase {
  ...
  public async setupQueryTest() {
    ...
    this.QueryTestApiBase = `${this.ServerBaseUrl}/${this.QueryTestApiBase}`;
    const rawRecords = [
  ...
  @JwtMongooseTestBase.Test("queryTestData")
  ...
  public async shouldLimitResults() {
    ...
    const nextResp: JSONResponse = (await got.get(`${this.ServerBaseUrl}${resp.links.next}`, {responseType: "json"})).body;
